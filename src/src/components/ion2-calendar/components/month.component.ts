import {
    Component, ChangeDetectorRef, Input, Output, EventEmitter, forwardRef, AfterViewInit,
    NgZone
} from '@angular/core';
...
export class MonthComponent implements ControlValueAccessor, AfterViewInit {
    ...
    @Input() id: any;
    @Input() fixedDayRange: { time: number, totalDays: number, endDay: string };
    ...
    onSelected(item: any, days: any[]) {
        ...
        this.zone.run(() => {
            ...
            if (this.pickMode === pickModes.FIXED_RANGE) {
                const totalTmpDays = this.fixedDayRange.totalDays;
                const start = moment(item.time).date();
                const end = start + totalTmpDays;
                const j = moment(item.time);
                const total = j.add(totalTmpDays, 'days').toDate().getTime();
                let finalDayObj = {
